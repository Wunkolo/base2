...
)
{
	...
	for( std::size_t i = 0; i < Length; ++i )
	{
		Output[i] = __builtin_bswap64(
			_pdep_u64(static_cast<std::uint64_t>(Input[i]), LSB8)
			| BinAsciiBasis
		);
	}
#else
	for( std::size_t i = 0; i < Length; ++i )
	{
		...
		Output[i]
			= ((((((static_cast<std::uint64_t>(Input[i]) * LSB8) & UniqueBit)
				  + CarryShift)
				 & MSB8)
				>> 7)
			   | BinAsciiBasis);
...
)
{
	...
	for( ; i < Length; i += 2 )
	{
		...
		__m128i Result
			= _mm_set1_epi16(*reinterpret_cast<const std::uint16_t*>(&Input[i])
			);
		...
		Result
			= _mm_blendv_epi8(_mm_set1_epi8('0'), _mm_set1_epi8('1'), Result);
#else
		constexpr std::uint64_t BinAsciiBasis = LSB8 * '0';
		constexpr std::uint64_t MSB8          = LSB8 << 7u;
		// Mask this last bit
	...
	for( ; i < Length; i += 8 )
	{
		...
		Bytes8 = _mm512_shuffle_epi8(
			Bytes8, _mm512_set_epi64(
						LSB8 * 7, LSB8 * 6, LSB8 * 5, LSB8 * 4, LSB8 * 3,
						LSB8 * 2, LSB8 * 1, LSB8 * 0
					)
		);
		// Get unique bits in each byte into a 64-bit mask
		const __mmask64 BitMask
			= _mm512_test_epi8_mask(Bytes8, _mm512_set1_epi64(UniqueBit));
...
)
{
	...
	for( ; i < Length; i += 2 )
	{
		...
		*reinterpret_cast<std::uint16_t*>(&Output[i])
			= _mm_movemask_epi8(ASCII);
	}
...
)
{
	...
	for( ; i < Length; i += 4 )
	{
		...
		*reinterpret_cast<std::uint32_t*>(&Output[i])
			= _mm256_movemask_epi8(ASCII);
	}
...
)
{
	...
	for( ; i < Length; i += 8 )
	{
		...
		const __mmask64 Binary
			= _mm512_test_epi8_mask(ASCII, _mm512_set1_epi8(0x01));
...
std::size_t Base2::Filter(std::uint8_t Bytes[], std::size_t Length)
{
	...
	{
		// Read in 16 bytes at once
		const __m256i Word256
			= _mm256_loadu_si256(reinterpret_cast<const __m256i*>(Bytes + i));

		// Check for valid bytes, in parallel
		const std::uint32_t BinaryTest = _mm256_movemask_epi8(_mm256_cmpeq_epi8(
	...
	{
		// Read in 16 bytes at once
		const __m128i Word128
			= _mm_loadu_si128(reinterpret_cast<const __m128i*>(Bytes + i));

		// Check for valid bytes, in parallel
		const __m128i BinaryTest = _mm_cmpeq_epi8(
			_mm_and_si128(Word128, _mm_set1_epi8(0xFE)), _mm_set1_epi8(0x30)
		);
		if( _mm_movemask_epi8(BinaryTest) == 0xFFFF )
		{
			// We have 16 valid ascii-binary bytes
			_mm_storeu_si128(reinterpret_cast<__m128i*>(Bytes + End), Word128);
			End += 16;
		}
		else
		{
